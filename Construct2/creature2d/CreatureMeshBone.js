 ...
 *****************************************************************************/
  ...
  // Construct2 fix:
 // There was a conflict with matrix libraies.  Changed vec3_,mat4_,quat_,etc... to vec3__,mat4__,quat__,etc... in this file and
 // in gl-matrix.js
...
function dualquat_()
{
	this.real = quat_.create();
	this.real[Q_W] = 0;
	
	this.imaginary = quat_.create();
	this.imaginary[Q_W] = 0;
	
	this.tmpQ1 = quat_.create();
};

dualquat_.prototype.reset = function()
{
  quat_.identity(this.real);
  this.real[Q_W] = 0;
  quat_.identity(this.imaginary);
  this.imaginary[Q_W] = 0;
  quat_.identity(this.tmpQ1);
};

dualquat_.prototype.createFromData = function(q0, t)
{
	this.real = q0;
	this.imaginary = quat_.create();
...
dualquat_.prototype.add = function(quat__in, real_factor, imaginary_factor)
{
	//real = real.add((quat__in.real.cpy().mul(real_factor)));
	//var tmpQ = quat_.clone(quat__in.real);
	quat_.copy(this.tmpQ1, quat__in.real);
	
	quat_.scale(this.tmpQ1, this.tmpQ1, real_factor);
	quat_.add(this.real, this.tmpQ1, this.real);
	
    //imaginary = imaginary.add(quat__in.imaginary.cpy().mul(imaginary_factor));
    //tmpQ = quat_.clone(quat__in.imaginary);
  quat_.copy(this.tmpQ1, quat__in.imaginary);
  quat_.scale(this.tmpQ1, this.tmpQ1, imaginary_factor);
	quat_.add(this.imaginary, this.tmpQ1, this.imaginary);
};

dualquat_.prototype.normalize = function()
{
	var norm = quat_.length(this.real);
	
	this.real = quat_.scale(this.real, this.real, 1.0 / norm);
	this.imaginary = quat_.scale(this.imaginary, this.imaginary, 1.0 / norm);
};

var v0 = vec3_.create();
var ve = vec3_.create();
var trans = vec3_.create();
var tmpVec1 = vec3_.create();
var tmpvec2_ = vec3_.create();
var tmpVec0 = vec3_.create();
var aVec = vec3_.create();
var rot = vec3_.create();

dualquat_.prototype.transform = function(p)
{
...
//        var tmpVec1 = v0.cpy().scl((float)imaginary.w);
        tmpVec1 = vec3_.scale(tmpVec1, v0, this.imaginary[Q_W]);
        
//        var tmpvec2_ = v0.cpy().crs(ve);
		tmpvec2_ = vec3_.cross(tmpvec2_, v0, ve);
        
        //var tmpVec0 = ve.cpy().scl(real.w);
        //trans = tmpVec0.sub(tmpVec1).add(tmpvec2_);
        //trans.scl(2.0f);
        
        tmpVec0 = vec3_.scale(tmpVec0, ve, this.real[Q_W]);
        
        aVec = vec3_.subtract(aVec, tmpVec0, tmpVec1);
        trans = vec3_.add(trans, aVec, tmpvec2_);
        trans = vec3_.scale(trans, trans, 2.0);

        //var rot = real.transform(p.cpy());
        rot = vec3_.transformQuat(rot, p, this.real);

        //return rot.add(trans);
        rot = vec3_.add(rot, rot, trans);
...
Utils.setAxisMatrix = function(xAxis, yAxis, zAxis)
{
	...
	retMat = mat4_.transpose(retMat, retMat);
	
	return retMat;
};

Utils.matrixToquat_ = function(mat_in)
{
	var retquat_ = quat_.create();
	var te = mat_in,
	...
	if ( trace > 0 ) {
  		...
  		retquat_[Q_W] = 0.25 / s;
  		retquat_[Q_X] = ( m32 - m23 ) * s;
  		retquat_[Q_Y] = ( m13 - m31 ) * s;
  		retquat_[Q_Z] = ( m21 - m12 ) * s;

	} else if ( m11 > m22 && m11 > m33 ) {

  		s = 2.0 * Math.sqrt( 1.0 + m11 - m22 - m33 );

  		retquat_[Q_W] = ( m32 - m23 ) / s;
  		retquat_[Q_X] = 0.25 * s;
  		retquat_[Q_Y] = ( m12 + m21 ) / s;
		retquat_[Q_Z] = ( m13 + m31 ) / s;

	} else if ( m22 > m33 ) {

  		s = 2.0 * Math.sqrt( 1.0 + m22 - m11 - m33 );

	  	retquat_[Q_W] = ( m13 - m31 ) / s;
	  	retquat_[Q_X] = ( m12 + m21 ) / s;
	  	retquat_[Q_Y] = 0.25 * s;
	  	retquat_[Q_Z] = ( m23 + m32 ) / s;

	} else {

  		s = 2.0 * Math.sqrt( 1.0 + m33 - m11 - m22 );

  		retquat_[Q_W] = ( m21 - m12 ) / s;
  		retquat_[Q_X] = ( m13 + m31 ) / s;
  		retquat_[Q_Y] = ( m23 + m32 ) / s;
  		retquat_[Q_Z] = 0.25 * s;
...
{
	var ret_vec = vec3_.fromValues(-vec_in[Q_Y], vec_in[Q_X], vec_in[Q_Z]);
...
{
	var dir = vec3_.clone(vec_in);
	dir = vec3_.normalize(dir, dir);
	
	var pep_dir = Utils.rotateVec_90(dir);
	
	var cur_tangent = vec3_.fromValues(dir[Q_X], dir[Q_Y], 0);
	var cur_normal = vec3_.fromValues(pep_dir[Q_X], pep_dir[Q_Y], 0);
	var cur_binormal = vec3_.fromValues(0, 0, 1);
	
	var cur_rotate = mat4_.create();
...
Utils.clamp = function(num, min, max) {
  ...
  var newVec1 = vec3_.create();
  var newvec2_ = vec3_.create();

Utils.vecInterp = function(vec1, vec2_, ratio)
{
	newVec1 = vec3_.scale(newVec1, vec1, 1.0 - ratio);
	newvec2_ = vec3_.scale(newvec2_, vec2_, ratio);
	
	var retVec = vec3_.create();
	retVec = vec3_.add(retVec, newVec1, newvec2_);
	
	return retVec;
};

Utils.vec2_Interp = function(vec_1, vec_2, ratio)
{
	var newVec1 = vec2_.create();
	var newvec2_ = vec2_.create();
	
	newVec1 = vec2_.scale(newVec1, vec_1, 1.0 - ratio);
	newvec2_ = vec2_.scale(newvec2_, vec_2, ratio);
	
	var retVec = vec2_.create();
	retVec = vec2_.add(retVec, newVec1, newvec2_);
...
function MeshBone(key_in, start_pt_in, end_pt_in, parent_transform)
{
    ...
    this.rest_parent_mat = mat4_.create();
    this.rest_parent_inv_mat = mat4_.create();
    this.rest_world_mat = mat4_.create();
    this.rest_world_inv_mat = mat4_.create();
    this.bind_world_mat = mat4_.create();
    this.bind_world_inv_mat = mat4_.create();
    this.parent_world_mat = mat4_.create();
    this.parent_world_inv_mat = mat4_.create();
    ...
    this.setParentWorldInvMat(mat4_.create());
    this.setParentWorldMat(mat4_.create());
    
    this.local_binormal_dir = vec3_.fromValues(0.0,0.0,1.0);
    this.tag_id = 0;
...
MeshBone.prototype.setRestParentMat = function(transform_in, inverse_in)
{
	this.rest_parent_mat = transform_in;
        if(inverse_in == null) {
            this.rest_parent_inv_mat = mat4_.clone(this.rest_parent_mat);
            //rest_parent_inv_mat.inv();
            mat4_.invert(this.rest_parent_inv_mat, this.rest_parent_inv_mat);
        }
        else {
            this.rest_parent_inv_mat = mat4_.clone(inverse_in);
        }
};
...
MeshBone.prototype.setLocalRestStartPt = function(world_pt_in)
{
  ...
  this.local_rest_start_pt = vec3_.create();
  this.local_rest_start_pt = vec3_.transformMat4(this.local_rest_start_pt, world_pt_in, this.rest_parent_inv_mat);
  this.calcRestData();
};
...
MeshBone.prototype.setLocalRestEndPt = function(world_pt_in)
{
  ...
  this.local_rest_end_pt = vec3_.create();
  this.local_rest_end_pt = vec3_.transformMat4(this.local_rest_end_pt, world_pt_in, this.rest_parent_inv_mat);
  this.calcRestData();
};
...
{
  //        if( quat_ernion.Dot(world_dq.real, ref_dq.real) < 0) {
  //if( world_dq.real.dot(ref_dq.real) < 0) {
  if(quat_.dot(this.world_dq.real, ref_dq.real) < 0) {
    //this.world_dq.real = world_dq.real.cpy().mul(-1);
    this.world_dq.real = quat_.scale(this.world_dq.real, this.world_dq.real, -1);
    //this.world_dq.imaginary = world_dq.imaginary.cpy().mul(-1);
    this.world_dq.imaginary = quat_.scale(this.world_dq.imaginary, this.world_dq.imaginary, -1);
  }
...
MeshBone.prototype.getWorldRestStartPt = function()
{
  ...
  var ret_vec = vec3_.create();
  ret_vec = vec3_.transformMat4(ret_vec, this.local_rest_start_pt, tmp_mat);
...
MeshBone.prototype.getWorldRestEndPt = function()
{
  ...
  var tmp_mat = this.rest_parent_mat;
  var ret_vec = vec3_.create();
  ret_vec = vec3_.transformMat4(ret_vec, this.local_rest_end_pt, tmp_mat);
...
{
  var cur_tangent = vec3_.fromValues(this.local_rest_dir[Q_X], this.local_rest_dir[Q_Y], 0);
  var cur_binormal = vec3_.fromValues(this.local_binormal_dir[Q_X], this.local_binormal_dir[Q_Y], this.local_binormal_dir[Q_Z]);
  var cur_normal = vec3_.fromValues(this.local_rest_normal_dir[Q_X], this.local_rest_normal_dir[Q_Y], 0);

  var cur_translate = mat4_.create();
  //cur_translate.setTranslation(local_rest_end_pt.x, local_rest_end_pt.y, 0);
  mat4_.translate(cur_translate, cur_translate, this.local_rest_end_pt);
  ...
  var cur_final = mat4_.create();
  cur_final = mat4_.multiply(cur_final, cur_translate, cur_rotate);

  //rest_world_mat = rest_parent_mat.cpy().mul(cur_final);
  this.rest_world_mat = mat4_.create();
  this.rest_world_mat = mat4_.multiply(this.rest_world_mat, this.rest_parent_mat, cur_final); 

  this.rest_world_inv_mat = mat4_.clone(this.rest_world_mat);
  this.rest_world_inv_mat = mat4_.invert(this.rest_world_inv_mat, this.rest_world_inv_mat);
  //Matrix4.Invert(ref rest_world_mat, out rest_world_inv_mat);

//  var world_rest_dir = getWorldRestEndPt().cpy().sub( getWorldRestStartPt());
  var world_rest_dir = vec3_.clone(this.getWorldRestEndPt());
  world_rest_dir = vec3_.subtract(world_rest_dir, world_rest_dir, this.getWorldRestStartPt());
  
  world_rest_dir = vec3_.normalize(world_rest_dir, world_rest_dir);
  this.world_rest_pos = this.getWorldRestStartPt();


  var bind_translate = mat4_.create();
  //bind_translate.setTranslation(getWorldRestStartPt().x, getWorldRestStartPt().y, 0);
  bind_translate = mat4_.translate(bind_translate, bind_translate, this.getWorldRestStartPt());

  var tVec = vec3_.create();
  tVec = vec3_.sub(tVec, this.getWorldRestEndPt(), this.getWorldRestStartPt());
  var bind_rotate = Utils.calcRotateMat(tVec);
  //Matrix4 cur_bind_final = bind_translate.cpy().mul(bind_rotate);
  var cur_bind_final = mat4_.create();
  cur_bind_final = mat4_.multiply(cur_bind_final, bind_translate, bind_rotate);

  this.bind_world_mat = mat4_.clone(cur_bind_final);
  this.bind_world_inv_mat = mat4_.clone(this.bind_world_mat);
  this.bind_world_inv_mat = mat4_.invert(this.bind_world_inv_mat, this.bind_world_inv_mat);
...
{
  var translate_parent = mat4_.create();
  translate_parent = mat4_.translate(translate_parent, translate_parent, this.getWorldEndPt());

  var tVec = vec3_.create();
  tVec = vec3_.subtract(tVec, this.getWorldEndpt(), this.getWorldStartPt());
  var rotate_parent = Utils.calcRotateMat(tVec);

  var final_transform = mat4_.create();
  final_transform = mat4_.multiply(final_transform, translate_parent, rotate_parent);

  var final_inv_transform = mat4_.clone(final_transform);
  //final_inv_transform.inv();
  final_inv_transform = mat4_.invert(final_inv_transform, final_inv_transform);
...
MeshBone.prototype.computeWorldDeltaTransforms = function()
{
  ...
  var cur_tangent = vec3_.fromValues(calc["first"][Q_X], calc["first"][Q_Y], 0);
  var cur_normal = vec3_.fromValues(calc["second"][Q_X], calc["second"][Q_Y], 0);
  var cur_binormal = vec3_.fromValues(this.local_binormal_dir[Q_X], this.local_binormal_dir[Q_Y], this.local_binormal_dir[Q_Z]);
  ...
  cur_translate = mat4_.translate(cur_translate, cur_translate, this.world_start_pt);
  ...
  this.world_delta_mat = mat4_.create();
//  world_delta_mat = (cur_translate.cpy().mul(cur_rotate)).mul(bind_world_inv_mat);
  this.world_delta_mat = mat4_.multiply(this.world_delta_mat, cur_translate, cur_rotate);
  this.world_delta_mat = mat4_.multiply(this.world_delta_mat, this.world_delta_mat, this.bind_world_inv_mat);


  //        quat_ernion cur_quat_ = XnaGeometry.quat_ernion.CreateFromRotationMatrix(world_delta_mat);
  //var tmpMat = mat3.create();
  //tmpMat = mat3.frommat4_(tmpMat, this.world_delta_mat);
  var cur_quat_ = Utils.matrixToquat_(this.world_delta_mat);


  var tmp_pos =  Utils.getMatTranslate(this.world_delta_mat);
  this.world_dq = new dualquat_();
  this.world_dq.createFromData(cur_quat_, tmp_pos);
...
{
  var tangent = vec3_.create();
  tangent = vec3_.subtract(tangent, end_pt, start_pt);
  tangent = vec3_.normalize(tangent, tangent);
...
{
  var tmp_dir = vec3_.create();
  //Vector3 tmp_dir = local_rest_end_pt.cpy().sub(local_rest_start_pt);
  tmp_dir = vec3_.subtract(tmp_dir, this.local_rest_end_pt, this.local_rest_start_pt);
  
  this.rest_length = vec3_.length(tmp_dir);
};
									...
									start_index_in, end_index_in)
{
	...
	this.uv_warp_local_offset = vec2_.fromValues(0,0);
	this.uv_warp_global_offset = vec2_.fromValues(0,0);
	this.uv_warp_scale = vec2_.fromValues(1,1);
	this.opacity = 100.0;
...
var accum_dq = new dualquat_();
var accum_mat = mat4_.create();
var final_pt = vec3_.create();
var tmp1 = vec3_.create();
var tmp2 = vec3_.create();
...
MeshRenderRegion.prototype.poseFinalPts = function(output_pts, output_start_index, bones_map)
{
  ...
  for(var i = 0, l = this.getNumPts(); i < l; i++) {
    var cur_rest_pt =
      vec3_.set(tmp1, this.store_rest_pts[0 + read_pt_index],
          this.store_rest_pts[1 + read_pt_index],
          this.store_rest_pts[2 + read_pt_index]);
      // vec3_.fromValues(this.store_rest_pts[0 + read_pt_index],
      //     this.store_rest_pts[1 + read_pt_index],
      //     this.store_rest_pts[2 + read_pt_index]);
    ...
    accum_dq.normalize();
    var tmp_pt = vec3_.set(tmp2, cur_rest_pt[Q_X], cur_rest_pt[Q_Y], cur_rest_pt[Q_Z]);
    // var tmp_pt = vec3_.fromValues(cur_rest_pt[Q_X], cur_rest_pt[Q_Y], cur_rest_pt[Q_Z]);
...
MeshRenderRegion.prototype.setUseLocalDisplacements = function(flag_in)
{
      ...
      && this.use_local_displacements)
  {
    ...
    for(var i = 0; i < this.getNumPts(); i++) {
      this.local_displacements.push (vec2_.create());
    }
  }
};
...
MeshRenderRegion.prototype.setUsePostDisplacements = function(flag_in)
{
      ...
      && this.use_post_displacements)
  {
    this.post_displacements = [];
    for(var i = 0; i < this.getNumPts(); i++) {
      this.post_displacements.push(vec2_.create());
    }
  }
};
...
MeshRenderRegion.prototype.getRestLocalPt = function(index_in)
{
  ...
  var return_pt = vec2_.fromValues(this.store_rest_pts[0 + read_pt_index],
      this.store_rest_pts[1 + read_pt_index]);
  return return_pt;
};
...
MeshRenderRegion.prototype.clearLocalDisplacements = function()
{
  for(var i = 0; i < this.local_displacements.length; i++) {
    this.local_displacements[i] = vec2_.create();
  }
};
...
MeshRenderRegion.prototype.clearPostDisplacements = function()
{
  for(var i = 0; i < this.post_displacements.length; i++) {
    this.post_displacements[i] = vec2_.create();
  }
};
...
MeshRenderRegion.prototype.runUvWarp = function()
{
  ...
  for(var i = 0; i < this.uv_warp_ref_uvs.length; i++) {
    var set_uv = vec2_.clone(this.uv_warp_ref_uvs[i]);
    
   
    set_uv = vec2_.subtract(set_uv, set_uv, this.uv_warp_local_offset);
    set_uv[Q_X] *= this.uv_warp_scale[Q_X];
    set_uv[Q_Y] *= this.uv_warp_scale[Q_Y];
    set_uv = vec2_.add(set_uv, set_uv, this.uv_warp_global_offset);
...
MeshRenderRegion.prototype.initUvWarp = function()
{
  ...
  for(var i = 0; i < this.getNumPts(); i++) {
    this.uv_warp_ref_uvs.push(vec2_.create());
    
    this.uv_warp_ref_uvs[i] = vec2_.fromValues(this.store_uvs[cur_uvs_index],
...
{
	this.uv_warp_global_offset = vec2_.create();
    this.uv_warp_local_offset = vec2_.create();
    this.uv_warp_scale = vec2_.fromValues(-1,-1);
    this.key = key_in;
    this.enabled = false;
};
  ...
  for(var i = 0; i < base_cache.length; i++) {
    ...
    if(set_region.getUseLocalDisplacements()) {
          ...
          && (end_data.getLocalDisplacements().length == displacements.length))
      {
        for(var j = 0; j < displacements.length; j++) {
          var interp_val = Utils.vec2_Interp(base_data.getLocalDisplacements()[j], 
    ...
    if(set_region.getUsePostDisplacements()) {
          ...
          && (end_data.getPostDisplacements().length == displacements.length))
      {
        ...
        for(var j = 0; j < displacements.length; j++) {
          var interp_val = Utils.vec2_Interp(base_data.getPostDisplacements()[j], 
...
CreatureModuleUtils.ReadPointsArray2DJSON = function(data, key)
{
  ...
  for (var i = 0; i < num_points; i++)
  {
    var cur_index = i * 2;
    ret_list.push(
        vec2_.fromValues(raw_array[0 + cur_index], raw_array[1 + cur_index]));
  }
...
{
  var raw_array = CreatureModuleUtils.getFloatArray(data[key]);
  return vec2_.fromValues(raw_array[0], raw_array[1]);
};
...
{
  var raw_array = CreatureModuleUtils.getFloatArray(data[key]);
  return vec3_.fromValues(raw_array[0], raw_array[1], 0);
};
...
CreatureModuleUtils.FillUVSwapCache = function(json_obj, key, start_time, end_time, cache_manager)
{
  ...
  for (var cur_time in base_obj)
  {
    ...
    for (var cur_name in cur_node)
    {
      ...
      if(use_uv == true) {
        var local_offset = CreatureModuleUtils.ReadVector2JSON(uv_node, "local_offset"); //ReadJSONvec2_(*uv_node, "local_offset");
        var global_offset = CreatureModuleUtils.ReadVector2JSON(uv_node, "global_offset"); //ReadJSONvec2_(*uv_node, "global_offset");
        var scale = CreatureModuleUtils.ReadVector2JSON(uv_node, "scale"); //ReadJSONvec2_(*uv_node, "scale");
...
CreatureModuleUtils.FillUVSwapCacheFlat = function(animUVList, start_time, end_time, cache_manager)
{
  ...
  for (var i = 0; i < animUVList.timeSamplesLength(); i++)
  {
    ...
    for (var j = 0; j < cur_node.uvSwapsLength(); j++)
    {
      ...
      cache_data.setEnabled(use_uv);
      if(use_uv == true) {
        var local_offset = CreatureModuleUtils.ReadVector2Flat(uv_node.localOffsetArray()); //ReadJSONvec2_(*uv_node, "local_offset");
        var global_offset = CreatureModuleUtils.ReadVector2Flat(uv_node.globalOffsetArray()); //ReadJSONvec2_(*uv_node, "global_offset");
        var scale = CreatureModuleUtils.ReadVector2Flat(uv_node.scaleArray()); //ReadJSONvec2_(*uv_node, "scale");
        cache_data.setUvWarpLocalOffset(local_offset);
        cache_data.setUvWarpGlobalOffset(global_offset);
        cache_data.setUvWarpScale(scale);
      }
...
Creature.prototype.InitDefaultData = function()
{
    ...
    this.boundary_min = vec2_.create();
    this.boundary_max = vec2_.create();
...
Creature.prototype.ComputeBoundaryMinMax = function()
{
	...
	var minPt = vec2_.fromValues(this.render_pts[firstIdx + 0], this.render_pts[firstIdx + 1]);
	var maxPt = vec2_.fromValues(minPt[0], minPt[1]);
...
CreatureManager.prototype.CreateAllAnimations = function(load_data, use_flat_data)
{
  var all_animation_names = [];

  if(use_flat_data) {
    for(var i = 0; i < animFlat.clipsLength(); i++)
    {
      all_animation_names.push(animFlat.clips(i).name());
    } 
  }
  else {
    all_animation_names = CreatureModuleUtils.GetAllAnimationNames (load_data);
  }

  for(var i = 0; i < all_animation_names.length; i++)
  {
    var cur_name = all_animation_names[i];
    this.AddAnimation(new CreatureAnimation(load_data, cur_name, use_flat_data));
  }
...
CreatureManager.prototype.AlterBonesByAnchor = function(bones_map, animation_name_in)
{
	...
	for(var cur_bone_key in bones_map)
	{
		...
		start_pt = vec3_.subtract(start_pt, start_pt, vec3_.fromValues(anchor_point[0], anchor_point[1], 0));
		end_pt = vec3_.subtract(end_pt, end_pt, vec3_.fromValues(anchor_point[0], anchor_point[1], 0));
